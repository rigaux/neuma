...
class SearchContext(JSONEncoder):
	...
	def toJSON(self):
		return {"type": self.type, "ref": self.ref,
			"pattern": self.pattern, "keywords": self.keywords,
			"search_type": self.search_type, "info_message": self.info_message}
	...
	def fromJSON(data):
		...
		sc.type = data["type"]
		sc.ref = data["ref"]
		sc.pattern = data["pattern"]
		sc.keywords = data["keywords"]
		sc.search_type = data["search_type"]
		sc.info_message = data["info_message"]
		return sc
